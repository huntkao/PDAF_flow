...
void SimWorld::moveTo(int step)
{
  ...
  {
    settle_counter_ = sim_.settle_frames;
  }
}

LensStatus SimWorld::lensStatus() const
{
  return {current_step_, settle_counter_ > 0};
}
...
float SimWorld::blurredTexture(float x, int radius) const
{
  ...
  {
    acc += texture(x + static_cast<float>(j));
  }
...
PdInput SimWorld::capture(const AfRequest& request)
{
  ...
  if (settle_counter_ > 0 && --settle_counter_ == 0)
  {
    current_step_ = target_step_;
  }
